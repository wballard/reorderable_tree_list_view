...
class TreePath {
  ...
  static List<Uri> generateIntermediatePaths(Uri path) {
    ...
    for (int i = 0; i < segments.length; i++) {
      if (i == 0) {
        ...
        final bool isStandardWebProtocol = _standardWebProtocols.contains(path.scheme);
        ...
        final bool hostAsSegment = path.host.isNotEmpty && 
        ...
        if (hostAsSegment) {
          if (i == 1 && segments[0] == path.host) {
            ...
            final String intermediatePath = segments.sublist(startIdx, i).join('/');
  ...
  static Uri? getParentPath(Uri path) {
    ...
    if (hostAsSegment) {
      ...
      final List<String> pathOnlySegments = path.pathSegments.where((String s) => s.isNotEmpty).toList();
      ...
      if (pathOnlySegments.isEmpty) {
        ...
        final String parentPath = pathOnlySegments.sublist(0, pathOnlySegments.length - 1).join('/');
        return Uri(
          scheme: path.scheme,
          host: path.host,
          path: '/$parentPath',
    ...
    // Normal URI handling
    final String parentPath = segments.sublist(0, segments.length - 1).join('/');
    return Uri(
      scheme: path.scheme,
      host: path.host,
      path: '/$parentPath',
