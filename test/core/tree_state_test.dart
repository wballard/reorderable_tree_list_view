...
void main() {
  group('TreeState', () {
    ...
    test('provides access to all nodes', () {
      ...
      expect(
        treeState.allNodes.any((TreeNode n) => n.path == Uri.parse('file://')),
        isTrue,
      );
      expect(
        treeState.allNodes.any(
          (TreeNode n) => n.path == Uri.parse('file://var'),
        ),
        isTrue,
      );
    });
    ...
    test('provides quick lookup by path', () {
      final TreeNode? node = treeState.getNodeByPath(
        Uri.parse('file://var/data/readme.txt'),
      );
      ...
      final TreeNode? folderNode = treeState.getNodeByPath(
    ...
    test('returns null for non-existent path', () {
      final TreeNode? node = treeState.getNodeByPath(
        Uri.parse('file://does/not/exist'),
      );
      expect(node, isNull);
    });

    test('gets children of a node', () {
      final List<TreeNode> rootChildren = treeState.getChildren(
        Uri.parse('file://'),
      );
      ...
      final List<TreeNode> varChildren = treeState.getChildren(
        Uri.parse('file://var'),
      );
    ...
    test('returns empty list for leaf nodes', () {
      final List<TreeNode> children = treeState.getChildren(
        Uri.parse('file://var/config.json'),
      );
    ...
    test('visible nodes respect expansion state', () {
      ...
      expect(
        visibleNodes.any(
          (TreeNode n) => n.path == Uri.parse('file://var/data'),
        ),
        isFalse,
      );
      expect(
        visibleNodes.any(
          (TreeNode n) => n.path == Uri.parse('file://var/config.json'),
        ),
        isFalse,
      );
      expect(
        visibleNodes.any(
          (TreeNode n) => n.path == Uri.parse('file://var/data/readme.txt'),
        ),
        isFalse,
      );
      ...
      expect(
        visibleNodes.any((TreeNode n) => n.path == Uri.parse('file://var')),
        isTrue,
      );

      // And other branches should be visible
      expect(
        visibleNodes.any((TreeNode n) => n.path == Uri.parse('file://usr')),
        isTrue,
      );
      expect(
        visibleNodes.any((TreeNode n) => n.path == Uri.parse('file://usr/bin')),
        isTrue,
      );
    });
    ...
    test('preserves sort order in visible nodes', () {
      ...
      for (int i = 0; i < visibleNodes.length; i++) {
        ...
        if (parentPath != null) {
          ...
          final int parentIndex = visibleNodes.indexWhere(
