...
void main() {
  group('TreeBuilder', () {
    ...
    test('handles multiple schemes', () {
      ...
      final List<String> schemes = nodes.map((TreeNode n) => n.path.scheme).toSet().toList();
    ...
    test('maintains hierarchical order', () {
      ...
      for (int i = 0; i < nodes.length; i++) {
        ...
        if (parentPath != null) {
          // Find parent in list
          final int parentIndex = nodes.indexWhere((TreeNode n) => n.path == parentPath);
    ...
    test('sorts siblings alphabetically', () {
      ...
      final List<TreeNode> leafNodes = nodes.where((TreeNode n) => n.isLeaf).toList();
    ...
    test('handles URIs with special characters', () {
      ...
      expect(nodes.any((TreeNode n) => n.displayName == 'file with spaces.txt'), isTrue);
      expect(nodes.any((TreeNode n) => n.displayName == 'special+chars'), isTrue);
      expect(nodes.any((TreeNode n) => n.displayName == 'test@file.txt'), isTrue);
    });
