...
void main() {
  group('ReorderableTreeListViewItem Theme Integration', () {
    ...
    testWidgets('should show connector lines when enabled', (WidgetTester tester) async {
      ...
      // Should have TreeConnectorPainter
      final Finder customPaints = find.byType(CustomPaint);
      bool hasTreeConnectorPainter = false;
      if (customPaints.evaluate().isNotEmpty) {
        for (final Element element in customPaints.evaluate()) {
          final CustomPaint customPaint = element.widget as CustomPaint;
          if (customPaint.painter is TreeConnectorPainter) {
            hasTreeConnectorPainter = true;
            break;
          }
        }
      }
    ...
    testWidgets('should hide connector lines when disabled', (WidgetTester tester) async {
      ...
      const TreeTheme theme = TreeTheme(showConnectors: false);
      ...
      // Should not have TreeConnectorPainter when showConnectors is false
      final Finder customPaints = find.byType(CustomPaint);
      bool hasTreeConnectorPainter = false;
      if (customPaints.evaluate().isNotEmpty) {
        for (final Element element in customPaints.evaluate()) {
