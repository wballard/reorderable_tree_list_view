...
void main() {
  group('ReorderableTreeListView Integration', () {
    ...
    testWidgets('uses ReorderableListView instead of ListView', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('renders all tree nodes with proper keys', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('each item has unique ValueKey based on node key', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
      ...
      final List<Widget> items = tester
          .widgetList<ReorderableTreeListViewItem>(itemFinder)
          .toList();
    ...
    testWidgets('passes through scroll controller', (
      WidgetTester tester,
    ) async {
      ...
      final ReorderableListView reorderableList = tester
          .widget<ReorderableListView>(reorderableListFinder);
    ...
    testWidgets('passes through padding and other properties', (
      WidgetTester tester,
    ) async {
    ...
    testWidgets('configures drag handles for desktop platforms', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('items are draggable', (WidgetTester tester) async {
          ...
          home: Scaffold(
            body: ReorderableTreeListView(
              ...
              itemBuilder: (BuildContext context, Uri path) => SizedBox(
                height: 50,
                child: ListTile(title: Text(TreePath.getDisplayName(path))),
              ),
            ),
          ),
        ),
      ...
      final Finder firstItemFinder = find
