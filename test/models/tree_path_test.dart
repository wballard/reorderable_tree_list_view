  ...
  group('TreePath', () {
    group('generateIntermediatePaths', () {
      ...
      test('generates correct paths for single segment', () {
        ...
        expect(intermediatePaths, equals(<Uri>[Uri.parse('file://')]));
      });
    ...
    group('calculateDepth', () {
      test('calculates depth for various paths', () {
        ...
        expect(
          TreePath.calculateDepth(Uri.parse('file://var/data')),
          equals(2),
        );
        expect(
          TreePath.calculateDepth(Uri.parse('file://var/data/readme.txt')),
          equals(3),
        );
      });
    ...
    group('getDisplayName', () {
      test('returns scheme for root paths', () {
        expect(
          TreePath.getDisplayName(Uri.parse('file://')),
          equals('file://'),
        );
        expect(
          TreePath.getDisplayName(Uri.parse('http://example.com')),
          equals('http://example.com'),
        );
        expect(
          TreePath.getDisplayName(Uri.parse('custom://')),
          equals('custom://'),
        );
      });
      ...
      test('returns last segment for non-root paths', () {
        ...
        expect(
          TreePath.getDisplayName(Uri.parse('file://var/data')),
          equals('data'),
        );
        expect(
          TreePath.getDisplayName(Uri.parse('file://var/data/readme.txt')),
          equals('readme.txt'),
        );
      });
      ...
      test('handles trailing slashes', () {
        expect(
          TreePath.getDisplayName(Uri.parse('file://var/')),
          equals('var'),
        );
        expect(
          TreePath.getDisplayName(Uri.parse('file://var/data/')),
      ...
      test('handles special characters in names', () {
        expect(
          TreePath.getDisplayName(Uri.parse('file://my%20folder')),
          equals('my folder'),
        );
        expect(
          TreePath.getDisplayName(Uri.parse('file://special%2Bchar')),
          equals('special+char'),
        );
      });
    });
    ...
    group('getParentPath', () {
      ...
      test('returns parent for non-root paths', () {
        expect(
          TreePath.getParentPath(Uri.parse('file://var')),
          equals(Uri.parse('file://')),
        );
        expect(
          TreePath.getParentPath(Uri.parse('file://var/data')),
          equals(Uri.parse('file://var')),
        );
        expect(
          TreePath.getParentPath(Uri.parse('file://var/data/readme.txt')),
          equals(Uri.parse('file://var/data')),
        );
      });
    ...
    group('isAncestorOf', () {
      test('returns true for direct ancestors', () {
        expect(
          TreePath.isAncestorOf(Uri.parse('file://'), Uri.parse('file://var')),
          isTrue,
        );

        expect(
          TreePath.isAncestorOf(
            Uri.parse('file://var'),
            Uri.parse('file://var/data'),
          ),
          isTrue,
      ...
      test('returns false for non-ancestors', () {
        ...
        expect(
          TreePath.isAncestorOf(Uri.parse('file://var'), Uri.parse('file://')),
          isFalse,
        ); // Child cannot be ancestor of parent
      });
